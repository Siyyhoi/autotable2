...
export default function AIChatPanel({ 
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: "welcome",
      role: "assistant",
      content: "สวัสดีครับ! ผมพร้อมช่วยจัดการตารางเรียนของคุณ ลองถามอะไรก็ได้นะครับ 😊",
      timestamp: new Date()
    }
  ]);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const assistantMessage: ChatMessage = {
        ...
        role: "assistant",
        content: data.explanation || data.ai_analysis || data.message || "ดำเนินการเรียบร้อย",
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-[450px] bg-white shadow-2xl border-l-2 border-indigo-200 flex flex-col z-50 animate-in slide-in-from-right duration-300">
      {/* Header */}
      ...
      <div className="p-3 bg-indigo-50 border-b border-indigo-100">
        <p className="text-xs text-gray-600 mb-2 font-semibold">💡 คำสั่งด่วน:</p>
            ...
            >
              ...
              {msg.role === "assistant" && msg.aiInsights && (
                <div className="mt-3 pt-3 border-t border-gray-200 space-y-2">
                  {/* Main Suggestion */}
                  {(msg.aiInsights.main_suggestion || msg.aiInsights.suggestion || msg.aiInsights.recommendation) && (
                  ...
                  {/* Warnings */}
                  {msg.aiInsights.warnings && msg.aiInsights.warnings.length > 0 && (
                    <div className="flex gap-2 items-start bg-orange-50 p-2 rounded-lg">
                      ...
                      <div className="text-xs text-orange-800">
                        ...
                        {msg.aiInsights.warnings.map((w: string, i: number) => (
                          <p key={i}>• {w}</p>
                        ))}
                      </div>
                    </div>
                  ...
                  {msg.aiInsights.what_happened && (
                    <div className="flex gap-2 items-start bg-blue-50 p-2 rounded-lg">
                      ...
                      <p className="text-xs text-blue-800">{msg.aiInsights.what_happened}</p>
                    </div>
                  )}
                </div>
              )}
