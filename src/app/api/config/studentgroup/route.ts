...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { error: "ดึงข้อมูลกลุ่มเรียนไม่สำเร็จ" },
      { status: 500 },
    );
  }
}
  ...
  try {
    ...
    if (!group_id || !group_name || group_count === undefined || !advisor) {
      return NextResponse.json(
        {
          error:
            "กรุณากรอกข้อมูลให้ครบ (รหัส, ชื่อกลุ่ม, จำนวนนักเรียน, อาจารย์ที่ปรึกษา)",
        },
        { status: 400 },
      );
    }
    ...
    if (existing) {
      return NextResponse.json(
        { error: `รหัสกลุ่มนี้ (${group_id}) มีอยู่แล้ว` },
        { status: 400 },
      );
    }
...
export async function PUT(req: Request) {
  try {
    ...
    if (!group_id) {
      return NextResponse.json(
        { error: "ไม่พบรหัสกลุ่มเรียน (group_id)" },
        { status: 400 },
      );
    }
    ...
    if (result.matchedCount === 0) {
      return NextResponse.json(
        { error: "ไม่พบข้อมูลกลุ่มเรียนที่ต้องการแก้ไข" },
        { status: 404 },
      );
    }
