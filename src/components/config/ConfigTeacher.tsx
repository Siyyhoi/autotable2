...
export default function ConfigTeacher({ onClose }: ConfigTeacherProps) {
  ...
  const fetchTeachers = async () => {
    ...
    try {
      ...
      if (res.ok) setTeachers((await res.json()) as Teacher[]);
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div>
                <h1 className="text-xl md:text-2xl font-bold text-slate-800 flex items-center gap-2">
                  <Users className="w-6 md:w-7 h-6 md:h-7 text-indigo-600" />{" "}
                  จัดการข้อมูลอาจารย์
                </h1>
                <p className="text-slate-500 text-sm">
                  จำนวน {teachers.length} ท่าน
                </p>
              </div>
            </div>
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              <div className="relative group flex-1">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-100"
                />
              </div>
              <button
                onClick={handleOpenAdd}
                className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2 rounded-lg flex items-center justify-center gap-2 shadow-lg shadow-indigo-200 whitespace-nowrap"
              >
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                {filtered.map((t) => (
                  <div
                    key={t.teacher_id}
                    className="bg-white rounded-xl border border-slate-200 p-5 hover:shadow-xl hover:border-indigo-200 transition-all relative overflow-hidden group"
                  >
          ...
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl p-6 animate-in zoom-in-95">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-lg font-bold text-slate-800">
                {isEditing ? "แก้ไขข้อมูล" : "เพิ่มอาจารย์ใหม่"}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                className="w-full py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 mt-2"
              >
                บันทึกข้อมูล
              </button>
            </form>
