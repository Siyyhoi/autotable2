...
export default function ConfigSubject({ onClose }: ConfigSubjectProps) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/config/subject?id=${id}`, {
        method: "DELETE",
      });
      if (res.ok) {
          ...
          <div className="p-4 md:p-6 border-b border-slate-100 bg-white sticky top-0 z-10 flex flex-col gap-4">
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div>
                <h1 className="text-xl md:text-2xl font-bold text-slate-800 flex items-center gap-2">
                  <BookOpen className="w-6 md:w-7 h-6 md:h-7 text-blue-600" />
                  จัดการข้อมูลรายวิชา
                </h1>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              ...
              <button
                ...
                className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-lg flex items-center justify-center gap-2 shadow-lg shadow-blue-200 transition-transform active:scale-95 whitespace-nowrap"
              >
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                {filteredSubjects.map((sub) => (
                  <div
                    key={sub.subject_id}
                    className="group bg-white rounded-xl border border-slate-200 p-5 hover:shadow-xl hover:border-blue-200 transition-all duration-300 relative overflow-hidden"
                  >
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl border border-white/20 animate-in zoom-in-95 duration-200 overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                <label className="block text-xs font-semibold text-slate-400 mb-3 text-center uppercase tracking-wider">
                  โครงสร้างหน่วยกิต
                </label>
