...
export default function ConfigStudentGroup({
  onClose,
}: ConfigStudentGroupProps) {
  ...
  const fetchGroups = async () => {
    ...
    try {
      ...
      if (res.ok) setGroups((await res.json()) as StudentGroup[]);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm(`ยืนยันลบกลุ่มเรียน ${id}?`)) return;
            ...
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div>
                <h1 className="text-xl md:text-2xl font-bold text-slate-800 flex items-center gap-2">
                  <GraduationCap className="w-6 md:w-7 h-6 md:h-7 text-emerald-600" />{" "}
                  จัดการกลุ่มเรียน
                </h1>
                <p className="text-slate-500 text-sm">
                  จำนวน {groups.length} กลุ่ม
                </p>
              </div>
            </div>
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              <div className="relative group flex-1">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-100"
                />
              </div>
              <button
                onClick={handleOpenAdd}
                className="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2 rounded-lg flex items-center justify-center gap-2 shadow-lg shadow-emerald-200 whitespace-nowrap"
              >
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                {filtered.map((g) => (
                  <div
                    key={g.group_id}
                    className="bg-white rounded-xl border border-slate-200 p-5 hover:shadow-xl hover:border-emerald-200 transition-all relative overflow-hidden group"
                  >
                      ...
                      <div className="space-y-2 mb-4">
                        <div className="flex justify-between items-center text-sm text-slate-600 bg-slate-50 p-2 rounded">
                          <span className="flex items-center gap-1">
                            <Users className="w-4 h-4 text-slate-400" /> จำนวน
                          </span>
          ...
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl p-6 animate-in zoom-in-95">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-lg font-bold text-slate-800">
                {isEditing ? "แก้ไขข้อมูล" : "เพิ่มกลุ่มเรียนใหม่"}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                className="w-full py-3 bg-emerald-600 text-white rounded-xl font-bold hover:bg-emerald-700 mt-2"
              >
                บันทึกข้อมูล
              </button>
            </form>
