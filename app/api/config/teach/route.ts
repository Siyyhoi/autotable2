...
export async function POST(req: Request) {
  try {
    ...
    if (!teacher_id || !subject_id) {
      return NextResponse.json({ error: "กรุณาระบุรหัสอาจารย์และรหัสวิชา" }, { status: 400 });
    }
    ...
    if (existing) {
      return NextResponse.json({ error: "ข้อมูลการสอนนี้มีอยู่แล้ว" }, { status: 400 });
    }
...
export async function DELETE(req: Request) {
  try {
    ...
    const result = await db.collection("Teach").deleteOne({ _id: new ObjectId(id) });
